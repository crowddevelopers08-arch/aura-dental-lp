...
const C = { green: '#1D4231', gold: '#D3BB71' };
...
const steps = [
  {
    ...
    desc: 'Our orthodontic specialists evaluate your smile and discuss your treatment goals.',
  },
  {
    num: '02',
    icon: 'biotech',
    ...
    desc: 'Using advanced 3D scanning technology, we create a digital model and simulate your future smile.',
  },
  {
    num: '03',
    icon: 'precision_manufacturing',
    iconBg: C.green,
    title: ['Customized Aligner', 'Fabrication'],
    desc: 'Your clear aligners are custom-made to fit your teeth perfectly.',
  },
  {
    num: '04',
    icon: 'dentistry',
  ...
  },
  {
    ...
    desc: 'Complete your treatment with beautifully aligned teeth and a confident smile.',
  },
] as const;
...
function ConnectorArrows() {
  return (
    <svg aria-hidden className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 1000 200" preserveAspectRatio="none">
      <defs>
        <marker id="ah-al" markerWidth="9" markerHeight="7" refX="8" refY="3.5" orient="auto">
          <polygon points="0 0, 9 3.5, 0 7" fill={C.gold} />
        </marker>
      </defs>
      <circle cx="172" cy="100" r="5" fill={C.gold} />
      <path d="M 172,100 C 183,162 217,162 228,100" stroke={C.gold} strokeWidth="2.5" fill="none" markerEnd="url(#ah-al)" />
      <circle cx="372" cy="100" r="5" fill={C.gold} />
      <path d="M 372,100 C 383,38 417,38 428,100"   stroke={C.gold} strokeWidth="2.5" fill="none" markerEnd="url(#ah-al)" />
      <circle cx="572" cy="100" r="5" fill={C.gold} />
      <path d="M 572,100 C 583,162 617,162 628,100" stroke={C.gold} strokeWidth="2.5" fill="none" markerEnd="url(#ah-al)" />
      <circle cx="772" cy="100" r="5" fill={C.gold} />
      <path d="M 772,100 C 783,38 817,38 828,100"   stroke={C.gold} strokeWidth="2.5" fill="none" markerEnd="url(#ah-al)" />
    </svg>
  );
}
...
function StepCircle({ step, size = 'lg' }: { step: typeof steps[number]; size?: 'lg' | 'sm' }) {
  ...
  const iconSize = size === 'lg' ? 'text-[40px]' : 'text-[24px]';
  ...
  return (
    ...
    >
      ...
      <span
        className={`material-symbols-outlined relative z-[1] ${iconSize} text-white`}
...
export function AlignerJourneySection() {
  return (
    <section id="journey" className="px-4 py-10 sm:px-6 md:px-[60px] md:py-14 lg:py-18" style={{ backgroundColor: '#ffffff' }}>
      <div className="mx-auto max-w-[1280px]">
        ...
        <AnimateOnScroll animation="fade-down" className="mb-8 text-center md:mb-10">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#1D4231]/30 bg-[#DDD5CA]/60 px-3 py-1">
            <span className="material-symbols-outlined text-[12px] text-[#1D4231]" style={{ fontVariationSettings: '"FILL" 1' }}>route</span>
            <span className="font-outfit text-[10px] font-semibold uppercase tracking-[0.14em] text-[#1D4231] sm:text-[11px]">Your Aligner Journey</span>
          </span>
