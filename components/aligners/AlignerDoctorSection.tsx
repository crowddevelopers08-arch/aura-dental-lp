        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-[44%_56%] md:gap-12 lg:gap-16">
          ...
          <AnimateOnScroll animation="fade-right" className="hidden md:flex md:flex-col md:gap-4">
            ...
            <div className="rounded-2xl bg-[#1D4231] px-5 py-4">
              ...
              <p className="font-outfit text-[12px] font-medium italic leading-[1.7] text-[#D3BB71] sm:text-[13px]">
                Every patient deserves a smile that feels natural, functions perfectly, and lasts a lifetime.
              </p>
              <p className="mt-2 font-outfit text-[11px] font-bold uppercase tracking-[0.16em] text-[#D3BB71]/70">
          ...
          <AnimateOnScroll animation="fade-left" delay={100}>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-[#1D4231]/20 bg-white px-3 py-1">
              ...
              <span className="font-outfit text-[10px] font-semibold uppercase tracking-[0.14em] text-[#1D4231] sm:text-[11px]">Meet Your Smile Specialist</span>
            </span>

            <h2 className="mt-3 font-outfit text-[24px] font-extrabold leading-[1.1] text-[#000000] sm:text-[28px] md:text-[30px] lg:text-[34px]">
              Dr. Siva Nagini Yalavarthi
            </h2>
            <p className="mt-1.5 font-outfit text-[14px] font-semibold text-[#1D4231] sm:text-[15px]">
              MDS – Prosthodontist &amp; Smile Design Expert
            </p>
            ...
            <div className="mb-5 grid grid-cols-2 gap-3">
              {stats.map((s) => (
                <div key={s.label} className="rounded-xl bg-[#1D4231] px-4 py-3 text-center">
                  <p className="font-outfit text-[22px] font-extrabold text-white sm:text-[26px]">{s.value}</p>
                  <p className="font-outfit text-[11px] font-medium text-[#D3BB71] sm:text-[12px]">{s.label}</p>
                </div>
              ))}
            </div>

            <p className="mb-5 font-outfit text-[12px] leading-[1.8] text-[#000000]/70 sm:text-[13px] lg:text-[14px]">
            ...
            <div className="mb-5">
              <p className="mb-2 font-outfit text-[10px] font-bold uppercase tracking-[0.18em] text-[#000000] sm:text-[11px]">Areas of Expertise</p>
                ...
                <div className="marquee-track flex items-center gap-2" style={{ width: 'max-content', animationDuration: '22s' }}>
                  {[...specializations, ...specializations].map((s, i) => (
                    <div key={i} className="flex shrink-0 items-center gap-1.5 rounded-full border border-[#1D4231]/15 bg-white px-3 py-1.5">
                      ...
                      <span className="font-outfit text-[11px] font-medium text-[#1D4231] sm:text-[12px]">{s}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <a
              href="#consultation"
              className="font-outfit inline-flex items-center gap-2 rounded-full bg-[#1D4231] px-6 py-3 text-[13px] font-semibold text-white shadow-md transition-colors hover:bg-[#1D4231] sm:text-[14px] md:px-8"
            >
