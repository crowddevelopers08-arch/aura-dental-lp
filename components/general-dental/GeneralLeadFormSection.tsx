...
export function GeneralLeadFormSection() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push('/general-dental/thank-you');
    } catch {
      ...
      <div className="mx-auto max-w-[860px]">
        <div className="mb-8 text-center">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#1D4231]/30 bg-[#1D4231]/8 px-3 py-1">
            ...
            <span className="font-outfit text-[10px] font-semibold uppercase tracking-[0.14em] text-[#1D4231] sm:text-[11px]">Book Your Consultation</span>
          </span>
          <h2 className="mt-3 font-outfit text-[22px] font-extrabold leading-[1.2] text-[#1D4231] sm:text-[26px] md:text-[30px]">
            Start Your Dental Care Journey <span className="text-[#D3BB71]">Today</span>
          </h2>
          <p className="mx-auto mt-2 max-w-[520px] font-outfit text-[12px] leading-[1.8] text-[#000000]/55 sm:text-[13px]">
            Fill in the form and our dental specialists will reach out to help you choose the right treatment for your smile.
          </p>
        </div>
        ...
        <div className="rounded-[1.75rem] bg-white p-6 shadow-xl ring-1 ring-[#1D4231]/8 sm:p-8 md:p-10">
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                <label className="mb-1.5 block font-outfit text-[9px] font-bold uppercase tracking-[0.16em] text-[#1D4231]">
                  Full Name
                </label>
                <input
                  type="text"
                  ...
                  autoComplete="name"
                  className="w-full rounded-xl border border-[#1D4231]/15 bg-[#DDD5CA]/50 px-4 py-3 font-outfit text-[13px] text-[#000000] outline-none transition placeholder:text-[#000000]/35 focus:border-[#1D4231]/40 focus:bg-white"
                />
              </div>
              <div>
                <label className="mb-1.5 block font-outfit text-[9px] font-bold uppercase tracking-[0.16em] text-[#1D4231]">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    value={form.concern}
                    onChange={handleChange}
                    className="w-full appearance-none rounded-xl border border-[#1D4231]/15 bg-[#DDD5CA]/50 px-4 py-3 font-outfit text-[13px] text-[#000000]/60 outline-none transition focus:border-[#1D4231]/40 focus:bg-white"
                  >
            ...
            {error && (
              <p className="text-center font-outfit text-[11px] font-semibold text-red-500">{error}</p>
            )}

            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-full bg-[#1D4231] py-3.5 font-outfit text-[13px] font-bold uppercase tracking-[0.1em] text-white transition-opacity hover:opacity-90 disabled:opacity-70"
            >
