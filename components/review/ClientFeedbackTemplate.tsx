...
type ClientFeedbackTemplateProps = {
  /** Where "Change rating" sends the patient back to. */
  reviewHref: string;
  /** Optional branch name shown under the logo and sent with the submission. */
  branchLabel?: string;
};

export function ClientFeedbackTemplate({ reviewHref, branchLabel }: ClientFeedbackTemplateProps) {
    ...
    <>
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-5 sm:p-7 md:p-8 w-full max-w-[95vw] sm:max-w-md md:max-w-lg mx-auto my-4">
          ...
          {branchLabel && (
            <p
              className="-mt-2 mb-4 sm:mb-5 text-center text-sm sm:text-base font-semibold tracking-wide uppercase"
